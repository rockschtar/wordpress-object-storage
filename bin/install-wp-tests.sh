#!/usr/bin/env bash
# Installs the WordPress core + test suite for the PHPUnit integration tests.
# Trimmed-down version of the wp-cli scaffold script.
...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
		# Version x.y.0 is tagged as x.y in svn.
	...
	# 'latest': resolve the current stable version via the wordpress.org API.
...
install_test_suite() {
	# Portable in-place sed for both GNU sed and macOS sed.
